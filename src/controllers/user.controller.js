...
const changeCurrentPassword=asyncHandler(async(req,res)=>{
   ...
   if(!isPasswordCorrect){
    throw new ApiError(400,"Invalud old password")
   }
   user.password=newPassword
   await user.save({validateBeforeSave:false})
   return res.status(200)
   .json(new ApiResponse(200,{},"Password changed succsefully"))
})
const getCurrentUser=asyncHandler(async(req,res)=>{
    return res
    .status(200)
    .json(200,req.user,"current user fetcheds succefully")
})
const updateAccountDetails=asyncHandler(async(req,res)=>{
    const {fullname,email}=req.body
    if(!fullname || !email){
        throw new ApiError(400,"All fieild are required")
    }
...
const updateUserAvatar=asyncHandler(async(req,res)=>{
   ...
   const user= await User.findByIdAndUpdate(
    ...
    return res.status(200)
    .json(
        new ApiResponse(200,user,"Avatar image updated succuesfully")
    )
})
const updateUserCoverImage=asyncHandler(async(req,res)=>{
    ...
    return res.status(200)
    .json(
        new ApiResponse(200,user,"Cover image updated succuesfully")
    )
})
export {
    registerUser,
